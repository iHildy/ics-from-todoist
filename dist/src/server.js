...
async function generateCalendarContent(projectId) {
    try {
        ...
        console.log(`[Calendar Generation] Successfully generated calendar content`);
        return icsContent;
    }
    catch (error) {
...
app.get("/:projectName-:projectId", (req, res) => {
    ...
    console.log(`[Redirect] Redirecting old URL format to /calendar/${projectId}`);
...
app.get("/calendar/:projectId", async (req, res) => {
    ...
    try {
        ...
        console.log(`[Request] Cache miss, generating fresh content for project ${projectId}`);
        ...
        console.log(`[Request] Successfully served calendar for project ${projectId}`);
    }
    catch (error) {
        console.error(`[Error] Failed to generate calendar for project ${projectId}:`, error);
        // Check if it's a 404 error from Todoist
        if (error instanceof Error && error.message.includes("404")) {
            res.status(404).json({
                error: "Project not found",
                message: "The specified Todoist project could not be found. Please check your project ID.",
            ...
            res.status(500).json({
                ...
                message: "There was an error generating your calendar. Please try again later.",
...
app.listen(port, () => {
    ...
    console.log(`[Server] Todoist API token ${process.env.TODOIST_API_TOKEN ? "is" : "is NOT"} configured`);
    console.log(`[Server] Cache duration set to ${CACHE_DURATION / 1000} seconds`);
